...
3. `the_open_source_way-guidebook-2.0-zh.md` 是本书中文译文，任何翻译更改请在此处执行。
4. 确保你即将要翻译的章节没有其他人正在翻译，具体方法是：
   1. 查看下方 `章节` 部分，没有打勾的章节是为被翻译完成的
   2. 查看 `Pull Request` 页面，你需要保证此章节没有 Pull Request 被打开（或处于 Draft 状态）
5. 接下来，确认接取某章的翻译，在你 Fork 到你自己的用户下的仓库中，将对应章节的标题翻译，然后提出 Pull Request，并将其转换为 Draft，确保其他人知道你接取了本章节的翻译。Pull Request 标题必须为：`Translation: xxx`，`xxx` 替换为章节的英语标题。
